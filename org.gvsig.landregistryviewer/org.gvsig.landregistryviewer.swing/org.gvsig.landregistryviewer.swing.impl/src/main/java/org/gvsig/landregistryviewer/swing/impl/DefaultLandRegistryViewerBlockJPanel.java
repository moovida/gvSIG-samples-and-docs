...
public class DefaultLandRegistryViewerBlockJPanel extends DefaultLandRegistryViewerBlockJPanelLayout
        implements
            LandRegistryViewerBlockJPanel {
    ...
    private static final Logger logger = LoggerFactory.getLogger(DefaultLandRegistryViewerBlockJPanel.class);

    private LandRegistryViewerBlock block;

    protected JButton accept = null;
    protected String columnNames[] = new String[]{"Creation Date", "Municipality code", "Piece of ground"};

    public DefaultLandRegistryViewerBlockJPanel( DefaultLandRegistryViewerSwingManager uimanager, LandRegistryViewerBlock block )
    ...
    public void setLandRegistryViewerBlock( LandRegistryViewerBlock block ) throws LandRegistryViewerException {
        ...
        if (block == null) {
            ...
            try {
                List<LandRegistryViewerParcel> parcels = this.block.getParcels();
                data = new String[parcels.size()][3];
                for( int row = 0; row < parcels.size(); row++ ) {
                    LandRegistryViewerParcel parcel = parcels.get(row);
                    data[row][0] = String.valueOf(parcel.getCreationDate());
                    data[row][1] = String.valueOf(parcel.getMunicipalityCode());
                    data[row][2] = parcel.getCode();
                }
    ...
    private void initComponents() throws LandRegistryViewerException {
        ...
        parcelsTable.setModel(new DefaultTableModel(data, columnNames));
    ...
    private void translateComponents() {
        ...
        columnNames = new String[]{i18nManager.getTranslation("_Creation_Date"), i18nManager.getTranslation("_Municipality_code"),
